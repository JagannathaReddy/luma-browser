...
import { resolveCaptureOptions, StepCapturePipeline, writeStepArtifacts, } from '../../session/capture.js';
export async function handleExecute(deps, request, output) {
    ...
    await locks.browser(request.browser, async () => {
        const recordVideoDir = capture.video && request.sessionId ? sessions.videoDir(request.sessionId) : null;
        if (request.connect) {
