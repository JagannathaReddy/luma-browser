...
import { encodeHostResponse, formatRpcError, HandleRegistry, invokeHostPathCall, invokeHostPathFlush, } from './playwright-rpc.js';
import { decodeSandboxBuffer } from './values.js';
export async function runSandboxedScript(source, { browser, onStdout = (data) => process.stdout.write(data), onStderr = (data) => process.stderr.write(data), timeoutMs = LIMITS.sandboxTimeoutMs, memoryLimitBytes = LIMITS.sandboxMemoryBytes, } = {}) {
...
function installHostFunctions(context, { browser, registry, onStdout, onStderr, deadline, lifecycle }) {
    const installPromise = (name, implementation) => installPromiseHostFunction(context, name, implementation, { deadline, lifecycle });
    ...
    installPromise('__hostPathFlush', async (handleHandle, stepsHandle, pathHandle, methodHandle, argsHandle) => {
        const handleId = context.getString(handleHandle);
...
function installPromiseHostFunction(context, name, implementation, { deadline, lifecycle } = {}) {
