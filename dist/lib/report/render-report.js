...
import { escapeHtml, formatBytes, formatDuration, formatMs, shortenUrl } from '../html/helpers.js';
import { consoleLevelClass, parseConsoleJsonl, summarizeConsole } from './console-summary.js';
import { harStatusClass, summarizeHar } from './har-summary.js';
...
export async function renderSessionReport(sessionDir, results) {
    const steps = await Promise.all(results.steps.map(async (step) => {
...
function buildConsolePanel(step, index) {
    ...
    const filterButtons = [
        ...
        .map(([level, label], buttonIndex) => `<button type="button" class="${buttonIndex === 0 ? 'active' : ''}" data-level="${level}">${escapeHtml(label)}</button>`)
        .join('');
    const lines = entries
        .map((entry) => {
...
//# sourceMappingURL=render-report.js.map