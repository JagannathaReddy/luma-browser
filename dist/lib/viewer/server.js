...
const HTML_CSP = "default-src 'self'; img-src 'self' data:; style-src 'self' 'unsafe-inline'; script-src 'self' 'unsafe-inline'; media-src 'self'; frame-src 'self'; frame-ancestors 'none'; base-uri 'none'";
...
async function handleRequest(request, response, { sessionsRoot, token }) {
    ...
    if (pathname === '/' || pathname === '/index.html') {
        ...
        sendHtml(response, await renderIndexHtml(sessions, { sessionsDir: sessionsRoot }), extraHeaders);
        return;
    }
    ...
    if (!remainder) {
        try {
            ...
            sendHtml(response, await renderSessionDetailHtml(detail, { sessionsDir: sessionsRoot }), extraHeaders);
        }
        catch {
