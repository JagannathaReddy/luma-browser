...
import { buildExecutePayload, filterFlags, parseOptions, resolveRenderReport, } from './cli/parse-options.js';
...
function checkDaemonCompatibility(status) {
    if (typeof status?.protocolVersion === 'number' && status.protocolVersion !== PROTOCOL_VERSION) {
        throw new Error(`Daemon protocol version ${status.protocolVersion} is incompatible with CLI ${PROTOCOL_VERSION}. Run "luma-browser stop" and retry.`);
    }
...
function wrongBinaryError(mode, command) {
    if (mode === 'orchestrator') {
        throw new Error(`"${command}" is an engine command — use luma-browser instead (e.g. luma-browser run script.js).`);
    }
    throw new Error(`"${command}" is an orchestrator command — use luma instead (e.g. luma ${command}).`);
}
export async function main(argv = process.argv.slice(2), { mode = 'engine' } = {}) {
    ...
    if (command === 'browser-stop') {
        ...
        if (!browser) {
            throw new Error(`Usage: ${mode === 'orchestrator' ? 'luma' : 'luma-browser'} browser-stop <name>`);
        }
