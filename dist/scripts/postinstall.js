...
function getNpmGlobalPaths() {
    try {
        ...
        return {
            ...
            nodeModulesDir: platform() === 'win32' ? join(prefix, 'node_modules') : join(prefix, 'lib', 'node_modules'),
        };
    }
    catch {
        return null;
...
function fixWindowsShims() {
    ...
    writeFileSync(join(globalPaths.binDir, `${packageName}.ps1`), `#!/usr/bin/env pwsh\r\n$basedir = Split-Path $MyInvocation.MyCommand.Definition -Parent\r\n& node "$basedir\\${relativeLauncher}" $args\r\nexit $LASTEXITCODE\r\n`);
