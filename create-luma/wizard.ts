...
function StepHeader({ title, step, total }) {
  return React.createElement(
    ...
    React.createElement(Text, { bold: true, color: 'cyan' }, 'luma-browser setup'),
    React.createElement(Text, { dimColor: true }, `Step ${step}/${total} · ${title}`),
  );
}
...
function DonePanel({ usedGlobal }) {
  return React.createElement(
    ...
    React.createElement(Text, null, ' • Quick test: luma-browser run examples/example.com.js'),
    React.createElement(Text, null, ' • Record QA: bash examples/session-demo.sh'),
    React.createElement(Text, null, ' • Sessions: luma session start --name demo'),
    React.createElement(Text, null, ' • Full help: luma --help / luma-browser --help'),
    React.createElement(Text, null, ' '),
    React.createElement(
      Text,
      { dimColor: true },
      usedGlobal
        ...
        : `No global install — use npx ${LUMA_NPM_PACKAGE} …`,
    ),
    React.createElement(Text, null, ' '),
    React.createElement(Text, null, 'Agent plugins auto-update when marketplace versions change.'),
    React.createElement(
      Text,
      null,
      ' • Claude Code — /plugin marketplace add JagannathaReddy/luma-browser',
    ),
    React.createElement(Text, null, ' • Cursor — install "luma-browser" from Marketplace'),
  );
}
...
function Wizard({ nonInteractive }) {
  ...
  if (step === 1) {
    ...
    if (hasLuma) {
      return React.createElement(
        Box,
        { flexDirection: 'column' },
        React.createElement(StepHeader, { title: 'Existing install', step: 1, total: totalSteps }),
        React.createElement(Text, { color: 'green' }, '✓ luma-browser is already on PATH'),
    ...
    return React.createElement(
      Box,
      { flexDirection: 'column' },
      React.createElement(StepHeader, { title: 'Install method', step: 1, total: totalSteps }),
      !globalOk
        ? React.createElement(
            Text,
            { color: 'yellow' },
            'ℹ npm global prefix is not writable — npx is recommended.',
          )
        : null,
      React.createElement(SelectInput, {
        items: [
  ...
  if (step === 2) {
    return React.createElement(
      Box,
      { flexDirection: 'column' },
      React.createElement(StepHeader, { title: 'Install packages', step: 2, total: totalSteps }),
