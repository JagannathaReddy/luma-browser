...
export function defaultCaptureOptions(overrides = {}) {
  return {
...
export class SessionManager {
  ...
  constructor({ sessionsRoot = sessionsDir } = {}) {
  ...
  async startSession({ name, browser, capture } = {}) {
  ...
  async endSession(sessionId, { renderReport = true } = {}) {
    return this.#finalizeSession(sessionId, 'closed', { renderReport });
  }

  async abortSession(sessionId, { renderReport = false } = {}) {
  ...
  async #finalizeSession(sessionId, status, { renderReport = false } = {}) {
    const meta = await this.#readMeta(sessionId);
    meta.status = status;
    ...
    const results = await writeSessionResults(dir, meta);
  ...
  async finishStep(sessionId, stepIndex, { success, error, artifacts } = {}) {
    const meta = await this.#readMeta(sessionId);
