  ...
  attachToPage(page) {
    ...
    const onResponse = async (response) => {
      ...
      try {
        ...
        // Response bodies may be unavailable for some resources.
      }
    };
...
export class StepCapturePipeline {
  ...
  constructor({ context, stepDir, capture, primaryPage, videoDir } = {}) {
...
export async function writeSessionResults(sessionDir, meta) {
  ...
  await writeFile(join(sessionDir, 'results.json'), `${JSON.stringify(results, null, 2)}\n`);
  return results;
}
