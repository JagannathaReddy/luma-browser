...
const EVAL_STRING_METHODS = new Set([
  'evaluate',
  'evaluateHandle',
  '$eval',
  '$$eval',
...
function normalizeMethodArgs(method, args) {
  if (!EVAL_STRING_METHODS.has(method) || args.length === 0) {
    return args;
  }
  ...
  if (typeof first === 'string') {
    return [compileEvalString(first), ...rest];
  }
  ...
  if (
    ...
    return [{ ...first, content: compileEvalString(first.content) }, ...rest];
  }
...
export async function invokeHostPathCall(registry, handleId, path, method, args) {
  const target =
    path.length === 0 ? registry.get(handleId) : resolvePathTarget(registry, handleId, path);
...
export async function invokeHostPathFlush(registry, handleId, steps, path, method, args) {
  ...
  for (const step of steps) {
    const stepTarget =
      step.objectPath.length === 0
        ? target
        : resolvePathTarget(registry, activeHandleId, step.objectPath);
    const value = await invokeHostCallOnTarget(
      registry,
      activeHandleId,
      stepTarget,
      step.method,
  ...
  const finalTarget =
    path.length === 0 ? target : resolvePathTarget(registry, activeHandleId, path);
...
async function invokeHostCallOnTarget(registry, handleId, target, method, args) {
  ...
  if (CALLBACK_METHODS.has(resolvedMethod)) {
    throw new Error(
      `"${method}" is not supported from sandbox scripts (callbacks cannot cross the sandbox boundary)`,
    );
  }
  ...
  const normalized = normalizeMethodArgs(resolvedMethod, deserialized);

  let result = fn.apply(target, normalized);
