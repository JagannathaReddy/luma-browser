...
export class BrowserManager {
  ...
  async ensureBrowser(
    ...
    if (existing?.type === 'launched') {
      ...
      if (sessionScoped) {
        throw new Error(
          `Cannot change browser options for "${name}" mid-session — pin headless / ignoreHTTPSErrors / video at session start`,
        );
      }
  ...
  #buildAutoConnectError(lastError) {
    const detail =
      lastError instanceof Error ? lastError.message : 'No debugger endpoint responded';
    return [
