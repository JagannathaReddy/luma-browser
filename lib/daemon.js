...
import { parseRequest, PROTOCOL_VERSION, serializeResponse } from './protocol.js';
...
async function shutdown(exitCode = 0) {
  ...
  shuttingDown = (async () => {
    ...
    // Close the listener first so new connections fail fast with ECONNREFUSED
    // instead of racing the slow browser-shutdown path.
    if (activeServer) {
...
async function start() {
  ...
  server = net.createServer((socket) => {
    if (shuttingDown) {
      ...
      socket.write(
        serializeResponse({ id: 'unknown', type: 'error', message: 'Daemon is shutting down' }),
        () => socket.end(),
      );
      return;
    }
