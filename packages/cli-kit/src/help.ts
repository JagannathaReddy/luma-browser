...
export function helpForMode(mode: CliMode): string {
...
export function isOrchestratorCommand(command: string | undefined): boolean {
  return command != null && ORCHESTRATOR_COMMANDS.has(command);
}

export function isEngineCommand(argv: string[]): boolean {
  if (argv.length === 0) {
    return false;
  }
  if (argv[0] === 'run') {
